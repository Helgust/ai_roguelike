...
static void add_guerd_sm(flecs::entity entity) //guard behavior
{
  entity.get([](StateMachine& sm)
    {
      //idle
      int idle = sm.addState(create_idle_state());
      //moveToPlayer
      int moveToPlayer = sm.addState(create_move_to_player_state());
      //moveToEnemy
      int moveToEnemy = sm.addState(create_move_to_enemy_state());
      //Heal
      int heal = sm.addState(create_healing_ally_state());

      sm.addTransition(create_player_available_transition(1.f), moveToPlayer, idle);
      sm.addTransition(create_negate_transition(create_player_available_transition(100.f)), idle, moveToPlayer);

      sm.addTransition(create_enemy_available_transition(3.f), idle, moveToEnemy);
      sm.addTransition(create_enemy_available_transition(3.f), moveToPlayer, moveToEnemy);
      sm.addTransition(create_negate_transition(create_player_available_transition(5.f)), moveToEnemy, moveToPlayer);

      //player heal less and cooldown and near
      //sm.addTransition(create_player_hitpoints_less_than_transition(60.f), moveToPlayer, heal);
      sm.addTransition(create_and_transition(create_player_hitpoints_less_than_transition(60.f),create_cooldown_transition()),
        moveToPlayer, heal);
      sm.addTransition(create_negate_transition(create_cooldown_transition()), heal, moveToPlayer);
    });
}

static void add_patrol_attack_sm(flecs::entity entity) //berserk behavior
{
  entity.get([](StateMachine& sm)
    {
...
static void add_patrol_attack_flee_healing_sm(flecs::entity entity) //healer behavior
{
  entity.get([](StateMachine& sm)
    {
      int patrol = sm.addState(create_patrol_state(3.f));
      ...
      sm.addTransition(create_and_transition(create_hitpoints_less_than_transition(60.f), create_cooldown_transition()), patrol, heal);
      sm.addTransition(create_negate_transition(create_cooldown_transition()), heal, patrol);

      sm.addTransition(create_and_transition(create_hitpoints_less_than_transition(60.f), create_cooldown_transition()), moveToEnemy, heal);
      sm.addTransition(create_negate_transition(create_cooldown_transition()), heal, moveToEnemy);

      sm.addTransition(create_and_transition(create_hitpoints_less_than_transition(60.f), create_cooldown_transition()), fleeFromEnemy, heal);
      sm.addTransition(create_negate_transition(create_cooldown_transition()), heal, fleeFromEnemy);
    });
}
...
static flecs::entity create_monster(flecs::world &ecs, int x, int y, uint32_t color, int team)
{
  return ecs.entity()
    .set(Position{x, y})
    .set(MovePos{x, y})
...
static void register_roguelike_systems(flecs::world &ecs)
{
  ...
  ecs.system<HealingCooldown>()
    .each([&](HealingCooldown &heal_cooldown)
      {
        if (heal_cooldown.cooldown == 0)
          heal_cooldown.cooldown = 10;
        else
          heal_cooldown.cooldown -= 1;
      });
}
...
static void process_actions(flecs::world &ecs)
{
  ...
  static auto healing = ecs.query<Action, Hitpoints>();
  // Process all actions
  ecs.defer([&]
  {
    ...
    healing.each([&](flecs::entity entity, const Action& a, Hitpoints& hp)
    {
      if (a.action == EA_HEAL)
        hp.hitpoints += 10;
    });
